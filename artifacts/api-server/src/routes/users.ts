...
import { profileUpdateSchema } from "@workspace/api-zod";
...
router.post("/me/api-key", requireAuth, async (req: AuthRequest, res, next) => {
  try {
    ...
    res.json({
      data: { apiKey },
      message:
        "Store this key securely — it will not be shown again. Use X-API-Key header to authenticate.",
    });
  } catch (err) {
    next(err);
  }
...
router.get("/workers", requireAuth, async (req: AuthRequest, res, next) => {
  try {
    ...
    const faydaFilter =
      query.faydaOnly !== "false" ? eq(users.faydaVerified, true) : undefined;
    ...
    if (query.category) {
      workers = workers.filter((w) =>
        (w.skills ?? []).some((s) =>
          s.toLowerCase().includes(query.category!.toLowerCase()),
        ),
      );
    }
    ...
    const data = workers.map((w) => ({
      ...
      rating:
        w.ratingCount > 0
          ? Math.round((w.ratingSum / w.ratingCount) * 10) / 10
          : 0,
    ...
    res.json({
      ...
      meta: {
        page: query.page,
        limit: query.limit,
        hasMore: workers.length === query.limit,
      },
    });
  } catch (err) {
    next(err);
  ...
  try {
      ...
      .where(
        and(
          ...
          query.employerType
            ? eq(
                users.employerType,
                query.employerType as (typeof EMPLOYER_TYPES)[number],
              )
            : undefined,
        ),
      )
    ...
    if (query.ministryCode) {
      employers = employers.filter(
        (e) => e.ministryCode === query.ministryCode,
      );
    }
    ...
    res.json({
      ...
      meta: {
        page: query.page,
        limit: query.limit,
        hasMore: employers.length === query.limit,
      },
    });
  } catch (err) {
    next(err);
...
router.get(
  "/:id",
  requireAuth,
  async (req: AuthRequest<{ id: string }>, res, next) => {
    try {
...
router.post("/sos", requireAuth, async (req: AuthRequest, res, next) => {
  try {
    ...
    console.log(
      `SOS Alert from ${req.user.userId}: ${message || "Emergency"} at ${location.address}`,
    );
...
router.post(
  ...
  async (req: AuthRequest<{ id: string }>, res, next) => {
    try {
      ...
      const { badgeType, issuer, expiryDate } = z
        .object({
