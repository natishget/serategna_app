...
const SKILL_OPTIONS = [
  ...
  "Driving", "Cooking", "Gardening", "Moving", "Security", "Painting",
];
...
export default function RegisterWorkerScreen() {
  ...
  const handleRegister = async () => {
    if (!name || !phone || !faydaId || selectedSkills.length === 0) return;
  ...
  if (done) {
    return (
      <View style={[styles.root, styles.doneCenter, { backgroundColor: colors.background }]}>
        ...
        <Text style={[styles.doneTitle, { color: colors.foreground }]}>Worker Registered!</Text>
        <Text style={[styles.doneText, { color: colors.mutedForeground }]}>
          {name} has been registered. Your 2% commission will apply to their jobs.
        </Text>
        <Pressable style={[styles.homeBtn, { backgroundColor: colors.primary }]} onPress={() => router.back()}>
          <Text style={styles.homeBtnText}>Register Another</Text>
        </Pressable>
        <Pressable onPress={() => router.replace("/(tabs)")}>
          <Text style={[styles.skipText, { color: colors.mutedForeground }]}>Back to Home</Text>
        </Pressable>
      </View>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={[styles.headerTitle, { color: colors.foreground }]}>{t("agent.register_worker")}</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      >
        <Text style={[styles.heading, { color: colors.primary, marginBottom: 12 }]}>Agent Worker Registration</Text>
        <View style={{ backgroundColor: '#E3F2FD', borderRadius: 10, padding: 12, marginBottom: 16 }}>
          <Text style={{ color: '#0F4C81', fontWeight: 'bold', marginBottom: 4 }}>How It Works</Text>
          <Text style={{ color: '#0F4C81', marginBottom: 4 }}>• You (the agent) are registering a worker under your management.</Text>
          <Text style={{ color: '#0F4C81', marginBottom: 4 }}>• For every job this worker completes, you automatically earn a 2% commission.</Text>
          <Text style={{ color: '#0F4C81' }}>• The worker will be linked to your agent profile for compliance and payout tracking.</Text>
        </View>

        {/* Commission Info */}
        <View style={[styles.commissionBanner, { backgroundColor: `${colors.primary}12`, borderColor: `${colors.primary}30` }]}>
          <Feather name="award" size={20} color={colors.primary} />
          <View style={styles.commissionInfo}>
            <Text style={[styles.commissionTitle, { color: colors.primary }]}>2% Commission</Text>
        ...
        <View style={styles.form}>
          ...
          <View style={styles.field}>
            <Text style={[styles.label, { color: colors.foreground }]}>Fayda ID</Text>
            <View style={[styles.inputRow, { borderColor: colors.border, backgroundColor: colors.card }]}>
          ...
          <View style={styles.field}>
            <Text style={[styles.label, { color: colors.foreground }]}>Skills (select all that apply)</Text>
              ...
              {SKILL_OPTIONS.map((skill) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Text style={[styles.skillText, { color: selected ? colors.primary : colors.mutedForeground }]}>
                      {skill}
                    </Text>
                  </Pressable>
                );
          ...
          <Pressable
            style={[
              styles.submitBtn,
              {
                ...
                opacity: (name && phone && faydaId && selectedSkills.length > 0 && !loading) ? 1 : 0.5,
              },
            ]}
            onPress={handleRegister}
            disabled={!name || !phone || !faydaId || selectedSkills.length === 0 || loading}
          >
            {loading ? (
...
const styles = StyleSheet.create({
  ...
  commissionDesc: { fontFamily: "Inter_400Regular", fontSize: 13, lineHeight: 18 },
  form: { gap: 20 },
  field: { gap: 8 },
